...
<?php if (count($down_hosts) > 0) { ?>
    ...
    <tr><th>Tag</th><th>Hostname</th><th width="150px">State</th><th>Duration</th><th>Attempts</th><th>Detail</th></tr>
<?php
...
//if($host['tag'] == "AEMobile-FT")
//		           $tagUrl = $nagios_hosts[0]['nagios_url'];
	//
	//
if($service['tag'] == "ARIHANT-SERVER")
    $tagUrl = $nagios_hosts[0]['nagios_url'];
else if($service['tag'] == "ARIHANT-PROD")
    $tagUrl = $nagios_hosts[1]['nagios_url'];
else if($service['tag'] == "ARIHANT-DR")
    $tagUrl = $nagios_hosts[2]['nagios_url'];
else if($service['tag'] == "ARIHANT-MW2")
    $tagUrl = $nagios_hosts[3]['nagios_url'];
else if($service['tag'] == "ARIHANT-SMARTAPI")
    $tagUrl = $nagios_hosts[4]['nagios_url'];
else if($service['tag'] == "ARIHANT-MW2-DR")
    $tagUrl = $nagios_hosts[5]['nagios_url'];



		echo "<td align='left'> <a href='".$tagUrl."' target='_blank'>". $tag . "</a></td>";
        echo "<td> <font color='#FFFFF'> {$host['hostname']}  <span class='controls'>";
		                NagdashHelpers::render('controls.php',[ "tag" => $host['tag'],
										                                                                            "host" => $host['hostname'],
																													                                                                                                                        "service" => '']);
						                        echo "</span></td>";

        echo "<td> <font color='#FFFFF'> <blink>{$nagios_host_status[$host['host_state']]}</blink></td>";
        echo "<td> <font color='#FFFFF'> {$host['duration']}</td>";
        echo "<td> <font color='#FFFFF'> {$host['current_attempt']}/{$host['max_check_attempts']}</td>";
        echo "<td class=\"desc\"> <font color='#FFFFF'> {$host['detail']}</td>";
        echo "</tr>";
    }
?>
    </table>
<?php } else { ?>
    <table class="widetable status_green"><tr><td><b><font color="#FFFFF">All hosts OK</b></td></tr></table>
<?php
}
...
<?php if (count($broken_services) > 0) { ?>
    ...
    <tr><th width='5%'>Tag</th><th width="25%">Hostname</th><th width="50%">Service</th><th width="10%">Duration</th><th width="5%">Attempt</th></tr>
<?php
...
if($service['tag'] == "ARIHANT-SERVER")
    ...
    $tagUrl = $nagios_hosts[5]['nagios_url'];
        ...
        echo "<tr>";
                echo "<td align='left'><a href='".$tagUrl."' target='_blank'>". $tag . "</a></td>";
        echo "<td>{$service['hostname']}  <span class='controls'>";
      ...
      "service" => $service['service_name']]);
                                                                        echo "</span></td>";
        echo "<td class='{$nagios_service_status_colour[$service['service_state']]} {$soft_style}'>{$blink_tag}{$service['service_name']}<span class='detail'>{$service['detail']}</span></td>";
...
if (count($known_services) > 0) { ?>
    <h5>Known Service Problems</h5>
    <table class="widetable known_service" id="known_services">
    <tr><th width='5%'>Tag</th><th width="25%">Hostname</th><th width="37%">Service</th><th width="18%">State</th><th width="10%">Duration</th><th width="5%">Attempt</th></tr>
<?php
...
if($service['tag'] == "ARIHANT-SERVER")
	...
	$tag = NagdashHelpers::print_tag($service['tag'], count($nagios_hosts));
		echo "<td align='left'><a href='".$tagUrl."' target='_blank'>".$tag."</td>";
